...
#pragma mark - skynet funcs
    ...
    static int reg_skynet_error(lua_State * L) {
        int n = lua_gettop(L);
        if (n <= 0) {
            return luaL_error(L, "skynet.error: only %d arguments, more expected!\n", n);
        }
    ...
    void register_skynet_libs(lua_State* L) {
        luaL_checkversion(L);

        // register skynet
        luaL_Reg lib_skynet_funcs[] = {
            { "time",       reg_skynet_time},
            { "error",      reg_skynet_error},
            { NULL, NULL },
        };

        luaL_register(L, "skynet", lib_skynet_funcs);

        // register skynet.crypt
        srandom(((int)random() << 8) ^ ((int)time(NULL) << 16));

        luaL_Reg lib_crypt_funcs[] = {
            { "hashkey", lhashkey },
        ...
        luaL_register(L, "skynet.crypt", lib_crypt_funcs);
    }
}
